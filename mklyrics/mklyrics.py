...
def main(lyricfile, musicfile, outputfile, starttime):
    ...
    while True:
        ...
        if key == "q":
            ...
            trackSetting = ET.SubElement(tracks, "tracksettings", {'name':'first'})
            for segment in lyrics._lyrics:
                block = ET.SubElement(root, "block")
                track = ET.SubElement(block, "track", {'ref':'first'})
                for line in segment:
                    e_line = ET.SubElement(track, "line")
                    for word in line:
                        e_word = ET.SubElement(e_line, "word")
                        for syl in word:
                            e_syl = ET.SubElement(e_word, "syllable", {'start'
                                : str(int(syl.start * 1000)), 'end' : str(int(syl.end * 1000)) })
                            e_syl.text = syl.string
